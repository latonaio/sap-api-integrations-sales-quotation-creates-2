...
import (
	...
	"io/ioutil"
	"net/http"
	"sap-api-integrations-sales-quotation-creates/SAP_API_Caller/requests"
	sap_api_output_formatter "sap-api-integrations-sales-quotation-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callSalesQuotationSrvAPIRequirementHeader("A_SalesQuotation", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesQuotationSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_SalesQuotation('%s')/to_Item", item.SalesQuotation)
	outputDataItem, err := c.callSalesQuotationSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callSalesQuotationSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
